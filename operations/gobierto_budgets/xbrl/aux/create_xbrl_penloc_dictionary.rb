...
# Generate XBRL PENLOC YAML dictionary to match XBRL budget line names with their elasticsearch equivalents
#
# bin/rails runner $DEV/populate-data-indicators/private_data/gobierto/xbrl/aux/create_xbrl_penloc_dictionary.rb <xbrl_file>
...
if ARGV.empty? || ARGV.include?('-h') || ARGV.include?('--help')
  ...
  puts "      Format:"
  puts "        rails runner $DEV/populate-data-indicators/private_data/gobierto/xbrl/aux/create_xbrl_penloc_dictionary.rb <xbrl_file>"
  puts "------------------------------\n"
  exit
end
...
file = File.open(xbrl_file_path) { |f| Nokogiri::XML(f) }
...
xbrl_contextRef_values.map do |attribute|
  (attribute =~ /IdsEconomicaIngresos.*/) ? xbrl_budget_lines_ids.store(attribute, { used: false } ) : nil
end.compact.uniq

partidas_gastos_area_functional = GobiertoBudgets::FunctionalArea.all_items[GobiertoBudgets::BudgetLine::EXPENSE]   # => 391

# --- Partidas gastos area functional ---

diccionario_partidas_gastos_area_functional = {}

partidas_gastos_area_functional.each do |key, value|
  diccionario_partidas_gastos_area_functional.store(key, { name: value , search_name: "IdsEconomicaIngresos_" + get_xbrl_id_from_budget_line_name(value) })
end

diccionario_partidas_gastos_area_functional = diccionario_partidas_gastos_area_functional.sort.to_h
...
expense_functionalarea_elastic_to_xbrl_dictionary = build_elastic_to_xbrl_dictionary(diccionario_partidas_gastos_area_functional, xbrl_budget_lines_ids)
...
if DEBUG_MODE
  File.open(File.join(__dir__, "expense_functionalarea_budget_lines.json"),"w") do |f|
    f.write JSON.pretty_generate(expense_functionalarea_elastic_to_xbrl_dictionary)
  end
end
...
expense_functionalarea_elastic_to_xbrl_dictionary.each do |key, value|
...
File.open(File.join(__dir__, "xbrl_penloc_dictionary.yml"),"w") do |file|
