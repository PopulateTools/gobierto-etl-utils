...
puts "[START] transform-planned-updated/run.rb with dictionary=#{xbrl_dictionary_path} data=#{xbrl_file_path} organization_id=#{organization_id} year=#{year} output=#{output_file_path}"
...
puts "[END] transform-planned-updated/run.rb output=#{output_file_path}"
